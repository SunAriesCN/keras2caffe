...
def convert(keras_model, caffe_net_file, caffe_params_file):
    ...
    for layer in keras_model.layers:
        ...
        if layer_type=='InputLayer':
            ...
            if kernel_size[0]==kernel_size[1]:
                ...
                if padding=='same':
                    pad=kernel_size[0]/2
                    caffe_net[name] = L.Convolution(caffe_net[outputs[bottom]], num_output=filters, 
                                                    kernel_size=kernel_size[0], pad=pad, stride=strides[0], convolution_param=param, group=filters)
            else:
                pad_h = pad_w = 0
                if padding=='same':
                    pad_h = kernel_size[0]/2
                    pad_w = kernel_size[1]/2
                    caffe_net[name] = L.Convolution(caffe_net[outputs[bottom]], num_output=filters, 
                                                    kernel_h=kernel_size[0], kernel_w=kernel_size[1], pad_h=pad_h, pad_w=pad_w, stride=strides[0], convolution_param=param, group=filters)
            ...
            if config['activation'] == 'relu6': #testing code
                config['activation'] = 'relu' #testing code
        ...
        #TODO
            ...
            print 'layer.input:',layer.input.name
            print 'layer.output:',layer.output.name
            print config
